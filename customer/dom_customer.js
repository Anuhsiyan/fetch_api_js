import { getCustomers, getAddresses, createCustomer, updateCustomer, deleteCustomer, createAddress, updateAddress, deleteAddress } from './api_customer.js';
...
function handleEditCustomer(event) {
  ...
  const customerName = customerItem.children[0].textContent.replace('Name: ', '');
  const customerPhone = customerItem.children[1].textContent.replace('Phone: ', '');
  const customerEmail = customerItem.children[2].textContent.replace('Email: ', '');
...
function addAddressForm(address) {
  ...
  addressItem.innerHTML = `
    <input type="hidden" class="address-id" value="${address ? address.id : ''}">
    <label for="house-number">House Number:</label>
    <input type="text" class="house-number" value="${address ? address.houseNumber : ''}">
  ...
  addressItem.querySelector('.delete-address').addEventListener('click', function() {
